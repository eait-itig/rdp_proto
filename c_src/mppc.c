...
typedef struct bit_stream wBitStream;

static wBitStream *BitStream_New(void);
...
static void BitStream_Shift(wBitStream *bs, uint32_t nbits);
static void BitStream_Write_Bits(wBitStream *bs, uint32_t bits, uint32_t nbits);
static void BitStream_Flush(wBitStream *bs);
static void BitStream_Attach(wBitStream *bs, const uint8_t *buf, size_t capacity);
...
struct s_MPPC_CONTEXT
{
	...
	uint8_t HistoryBuffer[65536];
	uint16_t MatchBuffer[32768];
	uint32_t CompressionLevel;
};

static const uint32_t MPPC_MATCH_TABLE[256] = {
                    ...
                    const uint8_t** ppDstData, uint32_t* pDstSize, uint32_t flags)
{
	...
	uint8_t* SrcPtr;
	uint32_t CopyOffset;
	uint32_t LengthOfMatch;
	uint32_t accumulator;
	uint8_t* HistoryPtr;
	uint8_t* HistoryBuffer;
	uint8_t* HistoryBufferEnd;
	uint32_t HistoryBufferSize;
	uint32_t CompressionLevel;
	wBitStream* bs;
	...
	if (flags & PACKET_FLUSHED)
	{
		mppc->HistoryOffset = 0;
		mppc->HistoryPtr = HistoryBuffer;
		bzero(HistoryBuffer, mppc->HistoryBufferSize);
	}
	...
	*pDstSize = (uint32_t)(HistoryPtr - mppc->HistoryPtr);
	*ppDstData = mppc->HistoryPtr;
	mppc->HistoryPtr = HistoryPtr;
	return 1;
}

int mppc_compress(MPPC_CONTEXT* mppc, const uint8_t* pSrcData, uint32_t SrcSize, uint8_t* pDstBuffer,
                  const uint8_t** ppDstData, uint32_t* pDstSize, uint32_t* pFlags)
{
	const uint8_t* pSrcPtr;
	...
	while (pSrcPtr < (pSrcEnd - 2))
	{
		...
		*HistoryPtr++ = *pSrcPtr++;
		...
		if (MatchPtr != (HistoryPtr - 1))
			mppc->MatchBuffer[MatchIndex] = (uint16_t)(HistoryPtr - HistoryBuffer);
...
void mppc_context_reset(MPPC_CONTEXT* mppc, BOOL flush)
{
	...
	bzero(&(mppc->HistoryBuffer), sizeof(mppc->HistoryBuffer));
	bzero(&(mppc->MatchBuffer), sizeof(mppc->MatchBuffer));
...
static void
BitStream_Free(wBitStream *bs)
{
	free(bs);
}

static void
BitStream_Prefetch(wBitStream *bs)
{
	bs->prefetch = 0;
	if (bs->pointer - bs->buffer + 4 < bs->capacity)
		bs->prefetch |= bs->pointer[4] << 24;
	if (bs->pointer - bs->buffer + 5 < bs->capacity)
		bs->prefetch |= bs->pointer[5] << 16;
	if (bs->pointer - bs->buffer + 6 < bs->capacity)
		bs->prefetch |= bs->pointer[6] << 8;
	if (bs->pointer - bs->buffer + 7 < bs->capacity)
		bs->prefetch |= bs->pointer[7] << 0;
}

static void
BitStream_Fetch(wBitStream *bs)
{
	bs->accumulator = 0;
	if (bs->pointer - bs->buffer + 0 < bs->capacity)
		bs->accumulator |= bs->pointer[0] << 24;
	if (bs->pointer - bs->buffer + 1 < bs->capacity)
		bs->accumulator |= bs->pointer[1] << 16;
	if (bs->pointer - bs->buffer + 2 < bs->capacity)
		bs->accumulator |= bs->pointer[2] << 8;
	if (bs->pointer - bs->buffer + 3 < bs->capacity)
		bs->accumulator |= bs->pointer[3] << 0;
	BitStream_Prefetch(bs);
}

static void
BitStream_Shift(wBitStream *bs, uint32_t nbits)
{
	if (nbits == 0)
		return;
	assert(nbits < 32);
	...
	if (bs->offset < 32) {
		...
		if (bs->offset) {
			...
			bs->accumulator |=
			    (bs->prefetch >> (32 - bs->offset)) & bs->mask;
...
static void
BitStream_Write_Bits(wBitStream *bs, uint32_t bits, uint32_t nbits)
{
	...
	if (bs->offset < 32) {
		...
		if (bs->offset) {
			bs->mask = (1 << bs->offset) - 1;
			bs->accumulator |=
			    ((bits & bs->mask) << (32 - bs->offset));
		}
	}
}

static void
BitStream_Flush(wBitStream *bs)
{
	if (bs->pointer - bs->buffer + 0 < bs->capacity)
		bs->pointer[0] = (bs->accumulator >> 24) & 0xFF;
	if (bs->pointer - bs->buffer + 1 < bs->capacity)
		bs->pointer[1] = (bs->accumulator >> 16) & 0xFF;
	if (bs->pointer - bs->buffer + 2 < bs->capacity)
		bs->pointer[2] = (bs->accumulator >> 8) & 0xFF;
	if (bs->pointer - bs->buffer + 3 < bs->capacity)
		bs->pointer[3] = (bs->accumulator >> 0) & 0xFF;
}

static void
BitStream_Attach(wBitStream *bs, const uint8_t *buf, size_t capacity)
{
	bs->position = 0;
	bs->buffer = buf;
	bs->offset = 0;
	bs->accumulator = 0;
	bs->pointer = (uint8_t *)buf;
