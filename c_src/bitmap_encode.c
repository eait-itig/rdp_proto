...
#define GETPIXEL32(d, x, y, w) (*(((uint32_t*)d) + ((y) * (w) + (x))))
    ...
    do { \
        ...
        { \
            ...
            { \
                temp = (0x4 << 5) | in_count; \
                out_uint8(in_s, temp); \
                out_uint8a(in_s, get_data(in_data), in_count); \
            } \
            else if (in_count < 256 + 32) \
            { \
                out_uint8(in_s, 0x80); \
    ...
    do { \
        ...
        { \
            ...
            { \
                ...
                out_uint8(in_s, temp); \
                temp = in_count * 2; \
                out_uint8a(in_s, get_data(in_data), temp); \
            } \
            else if (in_count < 256 + 32) \
            { \
                out_uint8(in_s, 0x80); \
                     ...
                     struct stream *temp_s)
{
    char *line;
    char *last_line;
    char *fom_mask = malloc(16384); /* good for up to 64K bitmap */
    int lines_sent;
    int pixel;
    int count;
    int color_count;
    ...
    if ((bpp == 15) || (bpp == 16))
    {
        ...
        out_count = end * 2;
        line = in_data + width * (height - 1) * 2;

        while (line_no >= 0)/* && out_count < 32768)*/
        {
            i = get_pos(s) + count * 2;
    ...
    } else if (bpp == 24)
    {
        mix = 0xffffff;
        out_count = end * 3;
        line = in_data + width * (height - 1) * 4;

        while (line_no >= 0)/* && out_count < 65536)*/
        {
            i = get_pos(s) + count * 3;
